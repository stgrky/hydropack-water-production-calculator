...
export default function Home() {
  ...
  const avgHumidity =
    cityData.reduce((acc, curr) => acc + curr.humidity, 0) / totalMonths;
  const avgTemperature =
    cityData.reduce((acc, curr) => acc + curr.temperature, 0) / totalMonths;
  const avgAwdEffectivenessNum =
    cityData.reduce(
      (acc, curr) => acc + parseFloat(curr.awgEffectiveness.replace("%", "")),
      0
    ) / totalMonths;
  ...
  const avgUnit =
    cityData.reduce(
      (acc, curr) => acc + curr[selectedUnit as keyof MonthData],
      0
    ) / totalMonths;

  // Get the label for the active unit to display in the header.
  const activeUnitLabel =
    unitOptions.find((option) => option.key === selectedUnit)?.label || "";
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {cityData.map((entry) => (
              <tr key={entry.month}>
                ...
                <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">
                  {entry.awgEffectiveness}
                </td>
                <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">
                  {entry[selectedUnit as keyof MonthData]}
                </td>
              </tr>
            ))}
